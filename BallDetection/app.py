...
def run_inference(
    ...
    *,
    ...
    # 3) build Detections for annotation
    ...
    # 6) draw trajectory & build string for CSV
    traj_str = ""
    ...
    # 7) CSV logging: one row per frame
    if save_csv:
        # header has 17 columns => 16 fields after frame_number
        if det_to_draw.xyxy.size == 0:
            # no detections → blank row
            writer.writerow([frame_number] + [""] * 16)
        else:
    ...
    with st.spinner("Running inference… this may take a while"):
        ...
        with open(CSV_PATH, "w", newline="") as f:
            ...
            writer.writerow([
                ...
                "c_x","c_y","x1","y1","x2","y2",
                "chosen_id","chosen_x1","chosen_y1","chosen_x2","chosen_y2","chosen_cx","chosen_cy","trajectory"
            ])
            sv.process_video(
                source_path=source,
                target_path=target,
